...
$arr = array();
$res = mysqli_query($con,"SELECT * FROM logs_add_walkin ORDER by date_occured DESC");
while($row = mysqli_fetch_assoc($res)){
    ...
    //for action message
    if($status==0){
        $row['action']= "Declined Account Request of ". $row['patient_name']." was added";
    }
    else{
        $row['action']= "Patient ". $row['patient_name']." was added";
    }
