...
$patientqry = "Select * from `medication_record` where `patient_type` = 'Senior' and `date_given` > NOW()-interval ".$time." order by `date_given` asc limit $start_from,$rpp";
...
if(mysqli_num_rows($result)>0){
    ...
    $page_qry = "Select * from `medication_record` where `patient_type` = 'Senior' and `date_given` > NOW()-interval ".$time." order by `date_given` asc";
