...
if($time == 'daily'){
    $time = '1 day';
    $sql = 'Select * from `medication_record` where `patient_type` = "PWD" and DATE_FORMAT(`date_given`,"%Y %M %d") = DATE_FORMAT(NOW(),"%Y %M %d") order by `date_given` asc';
}
elseif ($time == 'weekly'){
    $time = '1 week';
...
elseif ($time == 'annually'){
    ...
    $startdate = $datearr[0];
    $enddate = $datearr[1];
    $sql = 'Select * from `medication_record` where `patient_type` = "PWD" and date(date_given) BETWEEN date("'.$startdate.'") and date("'.$enddate.'") order by `date_given` asc';
}
$patientqry = $sql;
