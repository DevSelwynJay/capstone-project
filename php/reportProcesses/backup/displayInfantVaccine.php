...
$patientqry = "Select * from `vaccination_record` where `patient_type` = 'Infant' and `date_given` > NOW()-interval ".$time." limit $start_from,$rpp";
...
if(mysqli_num_rows($result)>0){
    ...
    $page_qry = "Select * from `vaccination_record` where `patient_type` = 'Infant' and `date_given` > NOW()-interval ".$time." ";
    ...
    if($total_records <= $rpp){
        ...
        for ($i = 1; $i <= $total_pages; $i++) {
            $patienttable .= '<span class="pagination_linkinfant" style="cursor:pointer;padding:6px;border:1px solid #ccc;"id="' . $i . '">' . $i . '</span>';
        }
    }
