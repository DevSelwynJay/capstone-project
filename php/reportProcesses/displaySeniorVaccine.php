...
$patientqry = "Select * from `vaccination_record` where `patient_type` = 'Senior' and `date_given` > NOW()-interval ".$time." order by `date_given` asc limit $start_from,$rpp";
...
if(mysqli_num_rows($result)>0){
    ...
    $page_qry = "Select * from `vaccination_record` where `patient_type` = 'Senior' and `date_given` > NOW()-interval ".$time." order by `date_given` asc";
    ...
    if($total_records <= $rpp){
        ...
        for ($i = 1; $i <= $total_pages; $i++) {
            $patienttable .= '<span class="pagination_linksenior" style="cursor:pointer;padding:6px;border:1px solid #ccc;"id="' . $i . '">' . $i . '</span>';
        }
    }
