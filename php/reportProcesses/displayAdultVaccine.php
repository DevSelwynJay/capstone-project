...
elseif ($time == 'weekly'){
    $time = '1 week';
    $sql = 'Select * from `vaccination_record` where `patient_type` = "Adult" and yearweek(`date_given`) = yearweek(NOW()) order by `date_given` asc';

}
elseif ($time == 'monthly'){
    $sql = 'Select * from `vaccination_record` where `patient_type` = "Adult" and MONTH(`date_given`) = MONTH(NOW()) order by `date_given` asc';
    //MONTH(`dateadded`) = MONTH(NOW())
    $time = '1 month';
}
elseif($time == 'quarterly'){
    $sql = 'Select * from `vaccination_record` where `patient_type` = "Adult" and QUARTER(`date_given`) = QUARTER(NOW()) order by `date_given` asc';
    $time = '1 quarter';
}
elseif ($time == 'annually'){
    $sql = 'Select * from `vaccination_record` where `patient_type` = "Adult" and YEAR(`date_given`) = YEAR(NOW()) order by `date_given` asc';
