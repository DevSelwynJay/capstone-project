...
$whereClause = "AND DATE_FORMAT(date_created,'%Y-%m-%d') = '$yesterday' ";
//para sa patient na may record
$whereClause2 = "AND DATE_FORMAT(date_given,'%Y-%m-%d') = '$yesterday' ";
...
//            echo "Both";
        if($lastMedConsultation>$lastVacConsultation){
            if($lastMedConsultation==$yesterday){
            ...
            if($lastVacConsultation==$yesterday){
